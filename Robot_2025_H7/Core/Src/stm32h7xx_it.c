...
void DMA1_Stream2_IRQHandler(void)
{
  /* USER CODE BEGIN DMA1_Stream2_IRQn 0 */
		static char c;
		if(RX_UART4[0]!=0xAA || RX_UART4[9] != 0x55)
			{
				for(c=0;c<9;c++){	RX_UART4[c]=0;	}// xoa data loi	
				HAL_NVIC_DisableIRQ(DMA1_Stream1_IRQn);
				HAL_NVIC_EnableIRQ(DMA1_Stream1_IRQn);
			}	
			
		else{
			 ball_x = (RX_UART4[1] << 8) | RX_UART4[2];
			 ball_y = (RX_UART4[3] << 8) | RX_UART4[4];
			 hoop_x = (RX_UART4[5] << 8) | RX_UART4[6];
			 hoop_y = (RX_UART4[7] << 8) | RX_UART4[8];
			
		}	
			//esp32_rx = (RX_UART4[0] << 8) | RX_UART4[1];
